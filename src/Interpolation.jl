...
# Masked version (e. g. to deal with air voxels).
"""
    interpolate_linearly(nodes::Juliana.ScalarGrid, p::Tuple{T, T, T})

Calculate the value at position p using trilinear interpolation and the given values on a regular grid.
"""
function interpolate_linearly(nodes::ScalarGrid, mask::ScalarGrid, p::Tuple{T, T, T}) where {T<:Real}
    # Implements the following algorithm:
    # https://en.wikipedia.org/wiki/Trilinear_interpolation
    # Origin of the "interpolation neighbourhood cube cell".
    ix = convert(Int64, floor((p[1] - nodes.grid.origin[1]) / nodes.grid.spacing[1])) + 1
    ...
    # Values at the closest interpolation points.
    c000 = nodes.data[ix  , iy  , iz  ] * mask.data[ix  , iy  , iz  ]
    c001 = nodes.data[ix  , iy  , iz+1] * mask.data[ix  , iy  , iz+1]
    c010 = nodes.data[ix  , iy+1, iz  ] * mask.data[ix  , iy+1, iz  ]
    c011 = nodes.data[ix  , iy+1, iz+1] * mask.data[ix  , iy+1, iz+1]
    c100 = nodes.data[ix+1, iy  , iz  ] * mask.data[ix+1, iy  , iz  ]
    c101 = nodes.data[ix+1, iy  , iz+1] * mask.data[ix+1, iy  , iz+1]
    c110 = nodes.data[ix+1, iy+1, iz  ] * mask.data[ix+1, iy+1, iz  ]
    c111 = nodes.data[ix+1, iy+1, iz+1] * mask.data[ix+1, iy+1, iz+1]
...
This function uses multi-threading to speed up the calculations.
"""
function interpolate_linearly(nodes::ScalarGrid, mask::ScalarGrid, points)
    results = Vector{eltype(points)}(undef, size(points, 2))
    @Threads.threads for i in 1:size(points, 2)
        @inbounds p = (points[1, i], points[2, i], points[3, i])
        @inbounds results[i] = Juliana.interpolate_linearly(nodes, mask, p)
    end
    return results
end


function interpolate_linearly(nodes::Juliana.ScalarGrid, mask::ScalarGrid, new_grid::Juliana.Grid)
    points, indices = Juliana.grid_to_points_and_indices(new_grid)
    values = convert.(Float32, Juliana.interpolate_linearly(nodes, mask, points))
    values[isnan.(values)] .= 0
    return Juliana.ScalarGrid(
        Juliana.flat_vector_to_cube(
            new_grid,
